module.exports = function toReadable (number) {
    ...
    const getFirstElemArray = (Object.keys(tens).map((elem) => elem.split(''))).map((arr) => arr[0]);

    // Получение первой цифры искомого числа (для двухзначных/трехзначных чисел)
    const findFirstElemNum = getFirstElemArray.find((elem) => elem === numArray[0]);
    ...
    const firstNumInWordsThree = ones[Object.keys(ones).find((n) => n === numString[0])];

    // Получaем вторую цифру искомого числа прописью для двухзначных чисел
    const secondNumInWordsTWo = ones[Object.keys(ones).find((n) => n === numString[1])];

    // Получaем вторую цифру искомого числа прописью для трехзначных чисел
    const secondNumInWordsThree = tens[`${Object.keys(ones).find((n) => n === numString[1])}0`];

    // Получaем третью цифру для трехзначных
    const thirdNumInWordsThree = ones[Object.keys(ones).find((n) => n === numString[2])];
